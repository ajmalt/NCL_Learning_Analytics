---
title: "2_2 Data description report"
...
```
* *country*, giving a two letter country code for each row (or 'Unknown' if the value is unknown - these values have been removed and replaced with NA, i.e. they are NULL). I assume this information originated from students on the course, and is therefore differentiated from *detected_country* (see below). The possible values in the data are:
```{r country levels, echo=FALSE}
levels(enrollmentData$country)
```
* *age_range*, giving - for each row in the data - an age range within which the user's age falls (or 'Unknown' if the value is unknown - these values have been removed and replaced with NA, i.e. they are NULL). The following age ranges are in the data:
```{r age range, echo=FALSE}
levels(enrollmentData$age_range)
```
* *highest_education_level*, giving - for each row in the data - the highest educaton level attained by the user at time of taking the course (or 'Unknown' if the value is unknown - these values have been removed and replaced with NA, i.e. they are NULL). I assume this has been inputted by the user. The following levels are in the data:
```{r education level, echo=FALSE}
levels(enrollmentData$highest_education_level)
```
* *employment_status*, giving - for each row in the data - the user's employment status at time of taking the course (or 'Unknown' if the value is unknown - these values have been removed and replaced with NA, i.e. they are NULL). I assume this has been inputted by the user. The following levels are in the data:
```{r employment status, echo=FALSE}
levels(enrollmentData$employment_status)
```
* *employment_area*, giving - for each row in the data - the sector in which the user was employed at time of taking the course (or 'Unknown' if the value is unknown - these values have been removed and replaced with NA, i.e. they are NULL). I assume this has been inputted by the user. The following levels are in the data:
```{r employment area, echo=FALSE}
levels(enrollmentData$employment_area)
```
* *detected_country*, giving (I assume) the country the system detects the user in when taking the course (or '--' if this value is unknown - these values have been removed and replaced with NA, i.e. they are NULL).
```{r detected country, echo=FALSE}
levels(enrollmentData$detected_country)
```


## Leaving survey responses

The leaving survey response data consists of seven files, three of which contained no rows of data (aside from column headings):

* cyber-security-1_leaving-survey-responses.csv - *empty*
* cyber-security-2_leaving-survey-responses.csv - *empty* 
* cyber-security-3_leaving-survey-responses.csv - *empty* 
